...
export function useSprayGumInventory() {
  // State management
  const [isSearching, setIsSearching] = useState(false);
  const [editingProduct, setEditingProduct] = useState<SprayGumProduct | null>(null);
  ...
  const uploadSelectedImage = useCallback(async (productId: number): Promise<string | null> => {
    ...
    try {
      ...
      const response = await fetch("/api/sprayGum/upload", {
        method: "POST",
        body: formData,
      });
  ...
  const handleEditSubmit = useCallback(async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const response = await fetch("/api/sprayGum", {
        method: "PUT",
        headers: {
  ...
  const handleAddToCart = useCallback((product: SprayGumProduct) => {
    // Convert SprayGumProduct to cart format (using ArduinoProduct structure for cart compatibility)
    const cartProduct = {
      id: product.id,
      ...
      inventoryType: "spraygum" as const,
    };
  ...
  const handleDelete = useCallback(async (product: SprayGumProduct) => {
