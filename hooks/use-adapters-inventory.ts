...
export function useAdaptersInventory() {
  // State management
  const [isSearching, setIsSearching] = useState(false);
  const [editingProduct, setEditingProduct] = useState<AdapterProduct | null>(null);
  ...
  const handleAddToCart = useCallback((product: AdapterProduct) => {
    addToCart({ ...product, barcode: product.barcode ?? null, inventoryType: "adapters" }, 1);
    showToast(
  ...
  const handleDelete = useCallback(async (product: AdapterProduct) => {
