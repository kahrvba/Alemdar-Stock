...
export function useLampsInventory() {
  // State management
  const [isSearching, setIsSearching] = useState(false);
  const [editingProduct, setEditingProduct] = useState<LampProduct | null>(null);
  ...
  const uploadSelectedImage = useCallback(async (productId: number): Promise<string | null> => {
    ...
    try {
      ...
      const response = await fetch("/api/lamps/upload", {
        method: "POST",
        body: formData,
      });
  ...
  const handleAddToCart = useCallback((product: LampProduct) => {
    addToCart({ ...product, barcode: product.barcode ?? null, inventoryType: "lamps" }, 1);
    showToast(
  ...
  const handleDelete = useCallback(async (product: LampProduct) => {
