...
export function useChargersInventory() {
  // State management
  const [isSearching, setIsSearching] = useState(false);
  const [editingProduct, setEditingProduct] = useState<ChargerProduct | null>(null);
  ...
  const uploadSelectedImage = useCallback(async (productId: number): Promise<string | null> => {
    ...
    try {
      ...
      const response = await fetch("/api/chargers/upload", {
        method: "POST",
        body: formData,
      });
  ...
  const handleAddToCart = useCallback((product: ChargerProduct) => {
    addToCart({ ...product, barcode: product.barcode ?? null, inventoryType: "chargers" }, 1);
    showToast(
  ...
  const handleDelete = useCallback(async (product: ChargerProduct) => {
