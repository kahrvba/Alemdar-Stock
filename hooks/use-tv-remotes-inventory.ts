...
export function useTvRemotesInventory() {
  const [isSearching, setIsSearching] = useState(false);
  const [editingProduct, setEditingProduct] = useState<TvRemote | null>(null);
  ...
  useEffect(() => {
    if (editingProduct) {
      setFormState({
        ...
        specs: JSON.stringify(editingProduct.specs ?? {}, null, 2),
  ...
  const uploadSelectedImage = useCallback(async (productId: number): Promise<string | null> => {
    ...
    try {
      ...
      const response = await fetch("/api/tv-remotes/upload", {
        method: "POST",
        body: formData,
      });
  ...
  const parseSpecs = useCallback(() => {
    if (!formState.specs?.trim()) {
      return {};
    }
    try {
      return JSON.parse(formState.specs);
    } catch {
  ...
  const handleEditSubmit = useCallback(async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const response = await fetch("/api/tv-remotes", {
        method: "PUT",
        headers: {
  ...
  }, [formState, editingProduct, selectedImageFile, imagePreviewUrl, uploadSelectedImage, showToast, router, parseSpecs]);
  ...
  }, [formState, selectedImageFile, imagePreviewUrl, uploadSelectedImage, showToast, router, parseSpecs]);

  const handleDelete = useCallback(async (product: TvRemote) => {
  ...
  const handleAddToCart = useCallback((product: TvRemote) => {
    const cartProduct = {
      id: product.id,
