...
export function useScrawesdriverInventory() {
  // State management
  const [isSearching, setIsSearching] = useState(false);
  const [editingProduct, setEditingProduct] = useState<ScrawesdriverProduct | null>(null);
  ...
  const uploadSelectedImage = useCallback(async (productId: number): Promise<string | null> => {
    ...
    try {
      ...
      const response = await fetch("/api/scrawesdriver/upload", {
        method: "POST",
        body: formData,
      });
  ...
  const handleEditSubmit = useCallback(async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const response = await fetch("/api/scrawesdriver", {
        method: "PUT",
        headers: {
  ...
  const handleAddToCart = useCallback((product: ScrawesdriverProduct) => {
    addToCart({ ...product, barcode: product.barcode ?? null, inventoryType: "scrawesdriver" }, 1);
    showToast(
  ...
  const handleDelete = useCallback(async (product: ScrawesdriverProduct) => {
