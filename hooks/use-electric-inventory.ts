...
export function useElectricInventory() {
  // State management
  const [isSearching, setIsSearching] = useState(false);
  const [editingProduct, setEditingProduct] = useState<ElectricProduct | null>(null);
  ...
  const uploadSelectedImage = useCallback(async (productId: number): Promise<string | null> => {
    ...
    try {
      ...
      const response = await fetch("/api/electric/upload", {
        method: "POST",
        body: formData,
      });
  ...
  const handleEditSubmit = useCallback(async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const response = await fetch("/api/electric", {
        method: "PUT",
        headers: {
  ...
  const handleDelete = useCallback(async (product: ElectricProduct) => {
