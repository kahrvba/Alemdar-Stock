...
export function useFanInventory() {
  // State management
  const [isSearching, setIsSearching] = useState(false);
  const [editingProduct, setEditingProduct] = useState<FanProduct | null>(null);
  ...
  const uploadSelectedImage = useCallback(async (productId: number): Promise<string | null> => {
    ...
    try {
      ...
      const response = await fetch("/api/fans/upload", {
        method: "POST",
        body: formData,
      });
  ...
  const handleAddToCart = useCallback((product: FanProduct) => {
    addToCart({ ...product, inventoryType: "fans" }, 1);
    showToast(
  ...
  const handleDelete = useCallback(async (product: FanProduct) => {
