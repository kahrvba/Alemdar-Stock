...
export function useOtherInventory() {
  ...
  const handleAddToCart = useCallback((product: OtherProduct) => {
    addToCart({ ...product, inventoryType: "others" }, 1);
    showToast(
  ...
  const handleDelete = useCallback(async (product: OtherProduct) => {
