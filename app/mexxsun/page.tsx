...
import { fetchMexxsunProducts, type MexxsunPagination } from "@/lib/services/mexxsun";
import { MexxsunInventoryClient } from "@/components/mexxsun/mexxsun-inventory-client";

export default async function MexxsunPage({
  searchParams,
}: {
  searchParams: Promise<{ page?: string; query?: string; field?: string }>;
}) {
  let responseData: MexxsunPagination | null = null;
  ...
  try {
    responseData = await fetchMexxsunProducts(apiBaseUrl, currentPage, query, field);
  } catch (error) {
    console.error("[mexxsun/page] fetch error:", error);
    return notFound();
  }
