  ...
  try {
    ...
    if (ids.length > 0) {
      ...
      const totalResult = await client.query(
        `SELECT COUNT(*) FROM public.electric ${whereClause}`,
        values
      );
      ...
      const result = await client.query(
        `SELECT *, COALESCE(quantity, 0) as quantity FROM public.electric ${whereClause} ORDER BY id ASC LIMIT $${values.length + 1} OFFSET $${values.length + 2}`,
...
export async function POST(req: Request) {
  ...
  try {
    ...
    let newId: number;
    try {
      const seqResult = await client.query('SELECT nextval(\'public.electric_id_seq\') as id');
      newId = Number(seqResult.rows[0]?.id);
    } catch {
      const maxIdResult = await client.query('SELECT COALESCE(MAX(id), 0) + 1 as next_id FROM public.electric');
...
export async function DELETE(req: Request) {
  ...
  try {
    ...
    // Delete the record
    await client.query('DELETE FROM public.electric WHERE id=$1', [id]);
    
    // Update the sequence to the current maximum ID
    await client.query(
      `SELECT setval(
        'public.electric_id_seq',
        GREATEST(COALESCE((SELECT MAX(id) FROM public.electric), 0), 1),
        COALESCE((SELECT MAX(id) FROM public.electric), 0) > 0
      );`
    );
