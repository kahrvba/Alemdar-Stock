...
export async function GET(req: Request) {
  ...
  try {
    ...
    if (id) {
      ...
      const result = await client.query(
        'SELECT id, name, rating, factory_price, wholesale_price, min_selling_price, selling_price, factor, cost_price, image_filename, category, barcode, COALESCE(quantity, 0) as quantity, description, COALESCE(is_new, false) AS is_new FROM public.mexxsun WHERE id = $1',
        [id]
      );
      client.release();
