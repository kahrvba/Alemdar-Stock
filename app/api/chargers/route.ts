  ...
  try {
    ...
    if (ids.length > 0) {
      ...
      if (query) {
        ...
        if (field && CHARGER_SEARCH_FIELDS.includes(field as ChargersSearchField)) {
          if (field === 'id') {
          ...
          whereClause = `WHERE (
            ${CHARGER_SEARCHABLE_EXPR} LIKE $1::text OR
            ($2::text IS NOT NULL AND ${CHARGER_SEARCHABLE_COMPACT_EXPR} LIKE $2::text) OR
      ...
      const totalResult = await client.query(
        `SELECT COUNT(*) FROM public.chargers ${whereClause}`,
        values
      );
      ...
      const result = await client.query(
        `SELECT *, COALESCE(quantity, 0) as quantity FROM public.chargers ${whereClause} ORDER BY id ASC LIMIT $${values.length + 1} OFFSET $${values.length + 2}`,
...
export async function POST(req: Request) {
  ...
  try {
    ...
    let newId: number;
    try {
      const seqResult = await client.query('SELECT nextval(\'public.chargers_id_seq\') as id');
      newId = Number(seqResult.rows[0]?.id);
    } catch {
      const maxIdResult = await client.query('SELECT COALESCE(MAX(id), 0) + 1 as next_id FROM public.chargers');
...
export async function DELETE(req: Request) {
  ...
  try {
    ...
    // Delete the record
    await client.query('DELETE FROM public.chargers WHERE id=$1', [id]);
    
    // Update the sequence to the current maximum ID
    await client.query(
      `SELECT setval(
        'public.chargers_id_seq',
        GREATEST(COALESCE((SELECT MAX(id) FROM public.chargers), 0), 1),
        COALESCE((SELECT MAX(id) FROM public.chargers), 0) > 0
      );`
    );
