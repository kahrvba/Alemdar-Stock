...
const SCRAWESDRIVER_SEARCH_FIELDS = ['id', 'english_names', 'turkish_names', 'category', 'barcode'] as const;
type ScrawesdriverSearchField = (typeof SCRAWESDRIVER_SEARCH_FIELDS)[number];

const COMPACT_REGEX = '[[:space:]/_.-]+';
const LAMP_SEARCHABLE_EXPR =
  "LOWER(COALESCE(english_names, '') || ' ' || COALESCE(turkish_names, '') || ' ' || COALESCE(category, '') || ' ' || COALESCE(barcode, ''))";
const LAMP_SEARCHABLE_COMPACT_EXPR =
  `REGEXP_REPLACE(${LAMP_SEARCHABLE_EXPR}, '${COMPACT_REGEX}', '', 'g')`;
...
export async function GET(req: Request) {
  ...
  try {
    ...
    if (ids.length > 0) {
      ...
      const result = await client.query(
        'SELECT *, COALESCE(quantity, 0) as quantity FROM public.scrawesdriver WHERE barcode = $1 ORDER BY id ASC',
        [barcode]
      );
      items = result.rows ?? [];
      total = items.length;
      ...
      if (query) {
        ...
        if (field && SCRAWESDRIVER_SEARCH_FIELDS.includes(field as ScrawesdriverSearchField)) {
          if (field === 'id') {
          ...
          whereClause = `WHERE (
            ${LAMP_SEARCHABLE_EXPR} LIKE $1::text OR
            ($2::text IS NOT NULL AND ${LAMP_SEARCHABLE_COMPACT_EXPR} LIKE $2::text) OR
      ...
      const totalResult = await client.query(
        `SELECT COUNT(*) FROM public.scrawesdriver ${whereClause}`,
        values
      );
      ...
      const result = await client.query(
        `SELECT *, COALESCE(quantity, 0) as quantity FROM public.scrawesdriver ${whereClause} ORDER BY id ASC LIMIT $${values.length + 1} OFFSET $${values.length + 2}`,
...
export async function POST(req: Request) {
  ...
  try {
    ...
    let newId: number;
    try {
      const seqResult = await client.query('SELECT nextval(\'public.scrawesdriver_id_seq\') as id');
      newId = Number(seqResult.rows[0]?.id);
    } catch {
      const maxIdResult = await client.query('SELECT COALESCE(MAX(id), 0) + 1 as next_id FROM public.scrawesdriver');
      newId = Number(maxIdResult.rows[0]?.next_id ?? 1);
    }

    const result = await client.query(
      'INSERT INTO public.scrawesdriver (id, english_names, turkish_names, category, barcode, quantity, price, image_filename, description) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9) RETURNING id',
...
export async function DELETE(req: Request) {
  ...
  try {
    ...
    // Delete the record
    await client.query('DELETE FROM public.scrawesdriver WHERE id=$1', [id]);
