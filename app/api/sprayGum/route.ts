  ...
  try {
    ...
    if (ids.length > 0) {
      const result = await client.query(
        'SELECT *, COALESCE(quantity, 0) as quantity FROM public.spray_gum WHERE id = ANY($1) ORDER BY id ASC',
        [ids]
      );
      client.release();
    ...
    if (barcode) {
      const result = await client.query(
        'SELECT *, COALESCE(quantity, 0) as quantity FROM public.spray_gum WHERE barcode = $1 ORDER BY id ASC',
        [barcode]
      );
      items = result.rows ?? [];
      total = items.length;
      ...
      const totalResult = await client.query(
        `SELECT COUNT(*) FROM public.spray_gum ${whereClause}`,
        values
      );
      ...
      const result = await client.query(
        `SELECT *, COALESCE(quantity, 0) as quantity FROM public.spray_gum ${whereClause} ORDER BY id ASC LIMIT $${values.length + 1} OFFSET $${values.length + 2}`,
...
export async function POST(req: Request) {
  ...
  try {
    ...
    let newId: number;
    try {
      const seqResult = await client.query("SELECT nextval('public.spray_gum_id_seq') AS id");
      newId = Number(seqResult.rows[0]?.id);
    } catch {
      console.warn('Sequence not found, using MAX(id) + 1 method');
      await client.query('LOCK TABLE public.spray_gum IN EXCLUSIVE MODE');
      const maxIdResult = await client.query('SELECT COALESCE(MAX(id), 0) + 1 AS next_id FROM public.spray_gum');
      newId = Number(maxIdResult.rows[0]?.next_id ?? 1);
    }

    await client.query(
      'INSERT INTO public.spray_gum (id, english_name, turkish_name, category, barcode, quantity, price, image_filename, description) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)',
