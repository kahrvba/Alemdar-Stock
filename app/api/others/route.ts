...
const OTHER_SEARCH_FIELDS = ['id', 'english_names', 'turkish_names', 'category', 'barcode'] as const;
type OtherSearchField = (typeof OTHER_SEARCH_FIELDS)[number];

const COMPACT_REGEX = '[[:space:]/_.-]+';
const OTHER_SEARCHABLE_EXPR =
  "LOWER(COALESCE(english_names, '') || ' ' || COALESCE(turkish_names, '') || ' ' || COALESCE(category, '') || ' ' || COALESCE(barcode, ''))";
const OTHER_SEARCHABLE_COMPACT_EXPR =
  `REGEXP_REPLACE(${OTHER_SEARCHABLE_EXPR}, '${COMPACT_REGEX}', '', 'g')`;
...
export async function GET(req: Request) {
  ...
  try {
    ...
    if (ids.length > 0) {
      ...
      const result = await client.query(
        'SELECT *, COALESCE(quantity, 0) as quantity FROM public.others WHERE barcode = $1 ORDER BY id ASC',
        [barcode]
      );
      items = result.rows ?? [];
      total = items.length;
      ...
      if (query) {
        ...
        if (field && OTHER_SEARCH_FIELDS.includes(field as OtherSearchField)) {
          if (field === 'id') {
          ...
          whereClause = `WHERE (
            ${OTHER_SEARCHABLE_EXPR} LIKE $1::text OR
            ($2::text IS NOT NULL AND ${OTHER_SEARCHABLE_COMPACT_EXPR} LIKE $2::text) OR
      ...
      const totalResult = await client.query(
        `SELECT COUNT(*) FROM public.others ${whereClause}`,
        values
      );
      ...
      const result = await client.query(
        `SELECT *, COALESCE(quantity, 0) as quantity FROM public.others ${whereClause} ORDER BY id ASC LIMIT $${values.length + 1} OFFSET $${values.length + 2}`,
...
export async function POST(req: Request) {
  ...
  try {
    ...
    let newId: number;
    try {
      const seqResult = await client.query('SELECT nextval(\'public.others_id_seq\') as id');
      newId = Number(seqResult.rows[0]?.id);
    } catch {
      const maxIdResult = await client.query('SELECT COALESCE(MAX(id), 0) + 1 as next_id FROM public.others');
      newId = Number(maxIdResult.rows[0]?.next_id ?? 1);
    }

    const result = await client.query(
      'INSERT INTO public.others (id, english_names, turkish_names, category, barcode, quantity, price, image_filename, description) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9) RETURNING id',
...
export async function DELETE(req: Request) {
  ...
  try {
    ...
    // Delete the record
    await client.query('DELETE FROM public.others WHERE id=$1', [id]);
    
    // Update the sequence to the current maximum ID
    await client.query(
      `SELECT setval(
        'public.others_id_seq',
        GREATEST(COALESCE((SELECT MAX(id) FROM public.others), 0), 1),
        COALESCE((SELECT MAX(id) FROM public.others), 0) > 0
      );`
    );
