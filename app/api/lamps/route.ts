...
const LAMP_SEARCH_FIELDS = ['id', 'english_names', 'turkish_names', 'category', 'barcode'] as const;
type LampsSearchField = (typeof LAMP_SEARCH_FIELDS)[number];
...
export async function GET(req: Request) {
  ...
  try {
    ...
    if (ids.length > 0) {
      ...
      const result = await client.query(
        'SELECT *, COALESCE(quantity, 0) as quantity FROM public.lamps WHERE barcode = $1 ORDER BY id ASC',
        [barcode]
      );
      items = result.rows ?? [];
      total = items.length;
      ...
      if (query) {
        ...
        if (field && LAMP_SEARCH_FIELDS.includes(field as LampsSearchField)) {
          if (field === 'id') {
      ...
      const totalResult = await client.query(
        `SELECT COUNT(*) FROM public.lamps ${whereClause}`,
        values
      );
      ...
      const result = await client.query(
        `SELECT *, COALESCE(quantity, 0) as quantity FROM public.lamps ${whereClause} ORDER BY id ASC LIMIT $${values.length + 1} OFFSET $${values.length + 2}`,
...
export async function POST(req: Request) {
  ...
  try {
    ...
    let newId: number;
    try {
      const seqResult = await client.query('SELECT nextval(\'public.lamps_id_seq\') as id');
      newId = Number(seqResult.rows[0]?.id);
    } catch {
      const maxIdResult = await client.query('SELECT COALESCE(MAX(id), 0) + 1 as next_id FROM public.lamps');
      newId = Number(maxIdResult.rows[0]?.next_id ?? 1);
    }

    const result = await client.query(
      'INSERT INTO public.lamps (id, english_names, turkish_names, category, barcode, quantity, price, image_filename, description) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9) RETURNING id',
...
export async function DELETE(req: Request) {
  ...
  try {
    ...
    // Delete the record
    await client.query('DELETE FROM public.lamps WHERE id=$1', [id]);
