...
import { fetchSprayGumProducts, type SprayGumPagination } from "@/lib/services/spray-gum";
import { SprayGumInventoryClient } from "@/components/spray-gum/spray-gum-inventory-client";

export default async function SprayGumPage({
  searchParams,
}: {
  searchParams: Promise<{ page?: string; query?: string; field?: string }>;
}) {
  let responseData: SprayGumPagination | null = null;
  ...
  try {
    responseData = await fetchSprayGumProducts(apiBaseUrl, currentPage, query, field);
  } catch (error) {
    console.error("[spray-gum/page] fetch error:", error);
    return notFound();
  }
