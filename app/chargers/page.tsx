...
import { fetchChargerProducts, type ChargerPagination } from "@/lib/services/chargers";
import { ChargersInventoryClient } from "@/components/chargers/chargers-inventory-client";
...
export default async function ChargersPage({
  ...
  let responseData: ChargerPagination | null = null;
  ...
  try {
    responseData = await fetchChargerProducts(apiBaseUrl, currentPage, query, field);
  } catch (error) {
    console.error("[chargers/page] fetch error:", error);
    return notFound();
  }
