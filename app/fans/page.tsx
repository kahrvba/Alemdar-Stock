...
import { fetchFanProducts, type FanPagination } from "@/lib/services/fans";
import { FanInventoryClient } from "@/components/fans/fans-inventory-client";
...
export default async function FanPage({
  ...
  let responseData: FanPagination | null = null;
  ...
  try {
    responseData = await fetchFanProducts(apiBaseUrl, currentPage, query, field);
  } catch (error) {
    console.error("[fans/page] fetch error:", error);
    return notFound();
  }
