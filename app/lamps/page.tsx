...
import { fetchLampProducts, type LampPagination } from "@/lib/services/lamps";
import { LampsInventoryClient } from "@/components/lamps/lamps-inventory-client";
...
export default async function LampsPage({
  ...
  let responseData: LampPagination | null = null;
  ...
  try {
    responseData = await fetchLampProducts(apiBaseUrl, currentPage, query, field);
  } catch (error) {
    console.error("[lamps/page] fetch error:", error);
    return notFound();
  }
