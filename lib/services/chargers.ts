...
const buildPaginatedResult = (
  data: PaginatedChargerResponse | ChargerProduct[],
  fallbackPage: number
): ChargerPagination => {
...
export async function fetchChargerProducts(
  baseUrl: string,
  page: number,
  query?: string | null,
  field?: string | null
): Promise<ChargerPagination> {
  const url = new URL("/api/chargers", baseUrl);
  ...
  if (!response.ok) {
    throw new Error(`Failed to fetch Charger products: ${response.status}`);
  }

  const data = (await response.json()) as
    | PaginatedChargerResponse
    | ChargerProduct[];
