...
const buildPaginatedResult = (
  data: PaginatedOtherResponse | OtherProduct[],
  fallbackPage: number
): OtherPagination => {
...
export async function fetchOtherProducts(
  ...
  query?: string | null,
  field?: string | null
): Promise<OtherPagination> {
  const url = new URL("/api/others", baseUrl);
  ...
  if (!response.ok) {
    throw new Error(`Failed to fetch Other products: ${response.status}`);
  }

  const data = (await response.json()) as
    | PaginatedOtherResponse
    | OtherProduct[];
