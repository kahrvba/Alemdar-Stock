...
const buildPaginatedResult = (
  data: PaginatedLampResponse | LampProduct[],
  fallbackPage: number
): LampPagination => {
...
export async function fetchLampProducts(
  baseUrl: string,
  page: number,
  query?: string | null,
  field?: string | null
): Promise<LampPagination> {
  const url = new URL("/api/lamps", baseUrl);
  ...
  if (!response.ok) {
    throw new Error(`Failed to fetch Lamp products: ${response.status}`);
  }

  const data = (await response.json()) as
    | PaginatedLampResponse
    | LampProduct[];
