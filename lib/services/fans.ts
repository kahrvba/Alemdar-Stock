...
const buildPaginatedResult = (
  data: PaginatedFanResponse | FanProduct[],
  fallbackPage: number
): FanPagination => {
...
export async function fetchFanProducts(
  ...
  query?: string | null,
  field?: string | null
): Promise<FanPagination> {
  const url = new URL("/api/fans", baseUrl);
  ...
  if (!response.ok) {
    throw new Error(`Failed to fetch Fan products: ${response.status}`);
  }
