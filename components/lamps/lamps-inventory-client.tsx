...
import { ProductCard } from "@/components/lamps/product-card";
import { PaginationControls } from "@/components/lamps/pagination-controls";
import { LampsSearch } from "@/components/lamps/lamps-search";
import type { LampProduct } from "@/lib/services/lamps";
import { cn } from "@/lib/utils";
import { BarcodeFieldWithGenerate } from "@/components/ui/barcode-field-with-generate";
import { useLampsInventory } from "@/hooks/use-lamps-inventory";
import { downloadExcel, highlightExcel } from "@/lib/excel-export";
import { Button } from "@/components/ui/button";
import { DropdownMenu, DropdownMenuContent, DropdownMenuItem, DropdownMenuTrigger, DropdownMenuSeparator } from "@/components/ui/dropdown-menu";
import { Checkbox } from "@/components/ui/checkbox";

type LampsInventoryClientProps = {
  items: LampProduct[];
  page: number;
  totalPages: number;
  previousPage: number;
...
export function LampsInventoryClient({
  items,
  page,
  totalPages,
  previousPage,
  ...
  const fetchAllProducts = async (): Promise<LampProduct[]> => {
    try {
      const response = await fetch("/api/lamps?pageSize=10000");
      if (!response.ok) {
            ...
            <div className="flex items-center gap-1">
              ...
              <span className="text-muted-foreground">Good (3)</span>
                ...
                <Suspense fallback={<div className="h-10 w-full" />}>
                  <LampsSearch onLoadingChange={setIsSearching} />
                </Suspense>
              </header>
