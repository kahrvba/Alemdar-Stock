...
import { ProductCard } from "@/components/spray-gum/product-card";
import { PaginationControls } from "@/components/spray-gum/pagination-controls";
import { CableSearch } from "@/components/spray-gum/spray-gum-search";
import type { SprayGumProduct } from "@/lib/services/spray-gum";
import { cn } from "@/lib/utils";
import { BarcodeFieldWithGenerate } from "@/components/ui/barcode-field-with-generate";
import { useSprayGumInventory } from "@/hooks/use-spray-gum-inventory";
...
type SprayGumInventoryClientProps = {
  items: SprayGumProduct[];
  page: number;
  totalPages: number;
  previousPage: number;
  nextPage: number;
...
export function SprayGumInventoryClient({
  items,
  page,
  totalPages,
  previousPage,
  ...
  const fetchAllProducts = async (): Promise<SprayGumProduct[]> => {
    try {
      const response = await fetch("/api/sprayGum?pageSize=10000");
      if (!response.ok) {
  ...
  const sendToArduino = useCallback(async (product: SprayGumProduct) => {
    if (!serialConnected) {
