...
import { ProductCard } from "@/components/fans/product-card";
import { PaginationControls } from "@/components/fans/pagination-controls";
import { FanSearch } from "@/components/fans/fans-search";
import type { FanProduct } from "@/lib/services/fans";
import { cn } from "@/lib/utils";
import { BarcodeFieldWithGenerate } from "@/components/ui/barcode-field-with-generate";
import { useFanInventory } from "@/hooks/use-fans-inventory";
import { downloadExcel, highlightExcel } from "@/lib/excel-export";
import { Button } from "@/components/ui/button";
import { DropdownMenu, DropdownMenuContent, DropdownMenuItem, DropdownMenuTrigger, DropdownMenuSeparator } from "@/components/ui/dropdown-menu";
import { Checkbox } from "@/components/ui/checkbox";

type FanInventoryClientProps = {
  items: FanProduct[];
  page: number;
  totalPages: number;
  previousPage: number;
...
export function FanInventoryClient({
  items,
  page,
  totalPages,
  previousPage,
  ...
  const fetchAllProducts = async (): Promise<FanProduct[]> => {
    try {
      const response = await fetch("/api/fans?pageSize=10000");
      if (!response.ok) {
            ...
            <div className="flex items-center gap-1">
              ...
              <span className="text-muted-foreground">Good (3)</span>
                ...
                <Suspense fallback={<div className="h-10 w-full" />}>
                  <FanSearch onLoadingChange={setIsSearching} />
                </Suspense>
              </header>
