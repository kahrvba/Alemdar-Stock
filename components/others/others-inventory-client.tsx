...
import { ProductCard } from "@/components/others/product-card";
import { PaginationControls } from "@/components/others/pagination-controls";
import { OtherSearch } from "@/components/others/others-search";
import type { OtherProduct } from "@/lib/services/others";
import { cn } from "@/lib/utils";
import { useOtherInventory } from "@/hooks/use-others-inventory";
import { downloadExcel, highlightExcel } from "@/lib/excel-export";
import { Button } from "@/components/ui/button";
import { DropdownMenu, DropdownMenuContent, DropdownMenuItem, DropdownMenuTrigger, DropdownMenuSeparator } from "@/components/ui/dropdown-menu";
import { Checkbox } from "@/components/ui/checkbox";

type OtherInventoryClientProps = {
  items: OtherProduct[];
  page: number;
  totalPages: number;
  previousPage: number;
...
export function OtherInventoryClient({
  items,
  page,
  totalPages,
  previousPage,
  ...
  const fetchAllProducts = async (): Promise<OtherProduct[]> => {
    try {
      const response = await fetch("/api/others?pageSize=10000");
      if (!response.ok) {
            ...
            <div className="flex items-center gap-1">
              ...
              <span className="text-muted-foreground">Good (3)</span>
                ...
                <Suspense fallback={<div className="h-10 w-full" />}>
                  <OtherSearch onLoadingChange={setIsSearching} />
                </Suspense>
              </header>
