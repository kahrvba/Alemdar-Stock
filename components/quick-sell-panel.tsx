...
import { HomeDbLookup } from "./home-db-lookup";
...
const makeItemKey = (item: { tableKey: string; id: number }) =>
...
export function QuickSellPanel() {
  ...
  const { formatFromUSD, formatMultiFromUSD, isLoadingRates } =
    useCurrencyRates();
  ...
  const hasInvalidSellQuantity = useMemo(
    () =>
      scannedItems.some(
        (item) =>
          item.sellQuantity < 1 ||
          item.sellQuantity > Math.max(0, item.quantity ?? 0),
      ),
    [scannedItems],
  );
  ...
  useEffect(() => {
    try {
      ...
      const hasLoadedKey =
        !!loadedActiveKey &&
        items.some((item) => makeItemKey(item) === loadedActiveKey);
      setActiveItemKey(hasLoadedKey ? loadedActiveKey : makeItemKey(items[0]));
    } catch (storageError) {
      console.error(
        "[quick-sell] failed to load persisted checkout list:",
        storageError,
      );
    } finally {
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.error(
        "[quick-sell] failed to persist checkout list:",
        storageError,
      );
    }
  ...
  useEffect(() => {
    ...
    if (
      !activeItemKey ||
      !scannedItems.some((item) => makeItemKey(item) === activeItemKey)
    ) {
  ...
  const lookupProduct = async (rawCode: string) => {
    ...
    try {
      const response = await fetch(
        `/api/quick-sell?code=${encodeURIComponent(trimmed)}`,
        {
          cache: "no-store",
      ...
      if (!response.ok) {
        const fail = (await response
          .json()
          .catch(() => null)) as QuickSellResolveResponse | null;
      ...
      const existing = scannedItems.find(
        (item) => makeItemKey(item) === incomingKey,
      );
      const maxAllowed = Math.max(1, resolvedItem.quantity ?? 0);
      const requestedQuantity = (existing?.sellQuantity ?? 0) + 1;
      const shouldShowOverLimitToast =
        requestedQuantity > maxAllowed && !!existing;
  ...
  useEffect(() => {
    ...
    window.addEventListener(
      BARCODE_COPIED_EVENT,
      handleBarcodeCopy as EventListener,
    );
  ...
  const updateActiveSellQuantity = (direction: "inc" | "dec") => {
    ...
    if (
      direction === "inc" &&
      activeItem.sellQuantity >= Math.max(0, activeItem.quantity ?? 0)
    ) {
      showToast("stock finished", "error");
      return;
    }
  ...
  const handleSell = async () => {
    ...
    try {
      ...
      const message =
        sellError instanceof Error ? sellError.message : "Sell failed";
  ...
  const handleFixZeroQuantity = async (item: ScannedQuickSellItem) => {
    ...
    try {
      ...
      const data = (await response
        .json()
        .catch(() => null)) as QuickSellFixResponse | null;
        ...
        current.map((entry) =>
          makeItemKey(entry) === itemKey
            ? {
                ...
                sellQuantity: Math.max(
                  1,
                  Math.min(entry.sellQuantity, fixedQuantity || 1),
                ),
              }
            : entry,
        ),
      ...
      const message =
        fixError instanceof Error ? fixError.message : "Failed to fix stock";
      showToast(message, "error");
    } finally {
  ...
  return (
    <>
      ...
      <aside className="w-full rounded-2xl border border-border/60 flex flex-col gap-2 bg-card p-4 lg:sticky lg:top-20">
        ...
        <input
          ...
          className={
            showManualInput
              ? "w-full h-11 rounded-xl border border-border/60 bg-background px-3 text-sm outline-none ring-0 transition focus:border-foreground/40"
              : "absolute -inset-full opacity-0 pointer-events-none"
          }
        />
            ...
            <div className="pt-4 space-y-2 flex flex-col h-full">
              <div className="space-y-1.5 flex-1 overflow-y-auto">
                {scannedItems.map((item) => {
                  const itemKey = makeItemKey(item);
                  const itemTotal =
                    (Number(item.price) || 0) * item.sellQuantity;
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <button
                          ...
                          onClick={() => {
                            setScannedItems((current) =>
                              current.filter(
                                (entry) => makeItemKey(entry) !== itemKey,
                              ),
                            );
                ...
                <div className="space-y-1 text-sm">
                  <div className="flex items-center justify-between text-muted-foreground">
                    ...
                    <span className="font-medium">
                      {formatFromUSD(discountedSubtotal, "USD")}
                    </span>
                  </div>
                  <div className="flex items-center justify-between text-muted-foreground">
                    <span>KDV ({Math.round(KDV_RATE * 100)}%)</span>
                    <span className="font-medium">
                      {formatFromUSD(kdvAmount, "USD")}
                    </span>
                  </div>
                  ...
                  <div className="flex items-start justify-between text-base font-bold text-foreground">
                    ...
                    <div className="mt-1 text-right">
                      <span>
                        {formatFromUSD(discountedGrandTotal, "USD")} (card)
                      </span>
                      <p>{formatFromUSD(discountedSubtotal, "USD")} (cash)</p>
                      <p className="text-sm font-medium text-muted-foreground">
                        {(() => {
                          const converted =
                            formatMultiFromUSD(discountedGrandTotal);
                      ...
                      <p className="text-sm font-medium text-muted-foreground">
                        {(() => {
                          const converted =
                            formatMultiFromUSD(discountedSubtotal);
