...
import { ProductCard } from "@/components/adapters/product-card";
import { PaginationControls } from "@/components/adapters/pagination-controls";
...
type AdaptersInventoryClientProps = {
...
export function AdaptersInventoryClient({
  items,
  page,
  totalPages,
  previousPage,
  ...
  const {
    ...
    handleAddToCart,
    handleDelete,
  } = useAdaptersInventory();

  const [isExporting, setIsExporting] = useState(false);
  const [highlightQuantity, setHighlightQuantity] = useState(false);

  const fetchAllProducts = async (): Promise<AdapterProduct[]> => {
    try {
      const response = await fetch("/api/adapters?pageSize=10000");
      if (!response.ok) {
            ...
            <div className="flex items-center gap-1">
              ...
              <span className="text-muted-foreground">Good (3)</span>
                ...
                <Suspense fallback={<div className="h-10 w-full" />}>
                  <AdaptersSearch onLoadingChange={setIsSearching} />
                </Suspense>
              </header>
