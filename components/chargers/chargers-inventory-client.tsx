...
import { ProductCard } from "@/components/chargers/product-card";
import { PaginationControls } from "@/components/chargers/pagination-controls";
import { ChargersSearch } from "@/components/chargers/chargers-search";
import type { ChargerProduct } from "@/lib/services/chargers";
import { cn } from "@/lib/utils";
import { useChargersInventory } from "@/hooks/use-chargers-inventory";
import { downloadExcel, highlightExcel } from "@/lib/excel-export";
import { Button } from "@/components/ui/button";
import { DropdownMenu, DropdownMenuContent, DropdownMenuItem, DropdownMenuTrigger, DropdownMenuSeparator } from "@/components/ui/dropdown-menu";
import { Checkbox } from "@/components/ui/checkbox";

type ChargersInventoryClientProps = {
  items: ChargerProduct[];
  page: number;
  totalPages: number;
  previousPage: number;
  nextPage: number;
...
export function ChargersInventoryClient({
  items,
  page,
  totalPages,
  previousPage,
  ...
  const {
    ...
    handleAddToCart,
    handleDelete,
  } = useChargersInventory();

  const [isExporting, setIsExporting] = useState(false);
  const [highlightQuantity, setHighlightQuantity] = useState(false);

  const fetchAllProducts = async (): Promise<ChargerProduct[]> => {
    try {
      const response = await fetch("/api/chargers?pageSize=10000");
      if (!response.ok) {
            ...
            <div className="flex items-center gap-1">
              ...
              <span className="text-muted-foreground">Good (3)</span>
                ...
                <Suspense fallback={<div className="h-10 w-full" />}>
                  <ChargersSearch onLoadingChange={setIsSearching} />
                </Suspense>
              </header>
