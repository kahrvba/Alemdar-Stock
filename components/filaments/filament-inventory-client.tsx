...
import { ProductCard } from "@/components/filaments/product-card";
...
type FilamentInventoryClientProps = {
...
export function FilamentInventoryClient({
  items,
  page,
  totalPages,
  previousPage,
      ...
      <div className="flex flex-col gap-12">
        <header className="flex flex-col items-start gap-4 text-left">
          ...
          <Suspense fallback={<div className="h-10 w-full" />}>
            <FilamentSearch onLoadingChange={setIsSearching} />
          </Suspense>
        </header>
          ...
          >
            {items.length ? (
              ...
              <div className="col-span-full flex h-48 flex-col items-center justify-center rounded-3xl border border-dashed border-border/60 text-sm text-muted-foreground">
                No filaments match this filter.
              </div>
            )}
          </div>
              ...
              <div className="grid gap-3 md:grid-cols-2">
                ...
                <label className="flex flex-col gap-1 text-sm text-muted-foreground">
                  ...
                  <input
                    ...
                    onChange={(event) => handleFormChange("material", event.target.value)}
                    className="rounded-2xl border border-border/60 bg-transparent px-3 py-2 text-sm text-foreground outline-none focus:border-primary"
                  />
                </label>
                <label className="flex flex-col gap-1 text-sm text-muted-foreground">
                  ...
                  <input
                    ...
                    onChange={(event) => handleFormChange("variant", event.target.value)}
                    className="rounded-2xl border border-border/60 bg-transparent px-3 py-2 text-sm text-foreground outline-none focus:border-primary"
                  />
                </label>
                <label className="flex flex-col gap-1 text-sm text-muted-foreground">
                ...
                <label className="flex flex-col gap-1 text-sm text-muted-foreground">
                  ...
                  <input
                    type="number"
                    step="0.01"
                    value={formState.net_weight_kg}
                    onChange={(event) => handleFormChange("net_weight_kg", event.target.value)}
                    className="rounded-2xl border border-border/60 bg-transparent px-3 py-2 text-sm text-foreground outline-none focus:border-primary"
                  />
                </label>
                <label className="flex flex-col gap-1 text-sm text-muted-foreground">
                  ...
                  <input
                    type="number"
                    step="0.01"
                    value={formState.diameter_mm}
                    onChange={(event) => handleFormChange("diameter_mm", event.target.value)}
                    className="rounded-2xl border border-border/60 bg-transparent px-3 py-2 text-sm text-foreground outline-none focus:border-primary"
                  />
                </label>
                <label className="flex flex-col gap-1 text-sm text-muted-foreground">
